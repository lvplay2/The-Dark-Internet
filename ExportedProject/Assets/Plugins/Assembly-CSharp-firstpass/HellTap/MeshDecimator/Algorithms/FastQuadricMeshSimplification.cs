	...
	public sealed class FastQuadricMeshSimplification : DecimationAlgorithm
	{
		...
		private bool Flipped(ref Vector3d p, int i0, int i1, ref Vertex v0, bool[] deleted)
		{
			...
			for (int j = 0; j < tcount; j++)
			{
				...
				Vector3d result;
				Vector3d.Cross(ref lhs, ref rhs, out result);
				result.Normalize();
				deleted[j] = false;
		...
		private void UpdateTriangles(int i0, int ia0, ref Vertex v, ResizableArray<bool> deleted, ref int deletedTriangles)
		{
			...
			for (int j = 0; j < tcount; j++)
			{
				...
				Vector3d result;
				int resultIndex;
				triangle.err0 = CalculateError(ref data2[triangle.v0], ref data2[triangle.v1], out result, out resultIndex);
		...
		private void RemoveVertexPass(int startTrisCount, int targetTrisCount, double threshold, ResizableArray<bool> deleted0, ResizableArray<bool> deleted1, ref int deletedTris)
		{
			...
			for (int i = 0; i < length; i++)
			{
				...
				for (int j = 0; j < 3; j++)
				{
					...
					Vector3d result;
					int resultIndex;
					CalculateError(ref data2[num3], ref data2[num4], out result, out resultIndex);
		...
		private void UpdateMesh(int iteration)
		{
			...
			for (int num19 = 0; num19 < num; num19++)
			{
				...
				Vector3d result;
				Vector3d.Cross(ref lhs, ref rhs2, out result);
				result.Normalize();
				data[num19].n = result;
			...
			for (int num20 = 0; num20 < num; num20++)
			{
				...
				Vector3d result2;
				int resultIndex;
				data[num20].err0 = CalculateError(ref data2[triangle.v0], ref data2[triangle.v1], out result2, out resultIndex);
		...
		private void CompactMesh()
		{
			...
			if (array != null)
			{
				vertNormals.Resize(length, true);
			}
			if (array2 != null)
			{
				vertTangents.Resize(length, true);
			}
			if (array3 != null)
			{
				vertUV2D.Resize(length, true);
			}
			if (array4 != null)
			{
				vertUV3D.Resize(length, true);
			}
			if (array5 != null)
			{
				vertUV4D.Resize(length, true);
			}
			if (array6 != null)
			{
				vertColors.Resize(length, true);
			}
			if (array7 != null)
			{
				vertBoneWeights.Resize(length, true);
			}
		}
		...
		public override void Initialize(Mesh mesh)
		{
			...
			for (int l = 0; l < 4; l++)
			{
				...
				string attributeName = string.Format("uv{0}", l);
				switch (uVDimension)
				{
				case 2:
				{
