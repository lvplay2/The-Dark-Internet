	...
	public sealed class Mesh
	{
			...
			for (int i = 0; i < num2; i++)
			{
				...
				if (array2 != null)
				{
					...
					for (int j = 0; j < num3; j += 3)
					{
						...
						Vector3 result;
						Vector3.Cross(ref lhs, ref rhs, out result);
						result.Normalize();
						array[num4] += result;
						array[num5] += result;
						array[num6] += result;
		...
		public void SetUVs(int channel, Vector3[] uvs)
		{
			...
			if (uvs != null && uvs.Length != 0)
			{
				...
				{
					throw new ArgumentException(string.Format("The vertex UVs must be as many as the vertices. Assigned: {0}  Require: {1}", num, vertices.Length), "uvs");
				}
				if (uvs3D == null)
				{
					uvs3D = new Vector3[4][];
		...
		public void SetUVs(int channel, List<Vector2> uvs)
		{
			...
			if (uvs != null && uvs.Count > 0)
			{
				...
				{
					throw new ArgumentException(string.Format("The vertex UVs must be as many as the vertices. Assigned: {0}  Require: {1}", count, vertices.Length), "uvs");
				}
				if (uvs2D == null)
				{
					uvs2D = new Vector2[4][];
