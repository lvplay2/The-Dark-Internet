	...
	public static class MeshDecimatorUtility
	{
		...
		public static UnityEngine.Mesh DecimateMeshes(UnityEngine.Mesh[] meshes, Matrix4x4[] transforms, Material[][] materials, Transform[][] meshBones, float quality, bool recalculateNormals, out Material[] resultMaterials, out Transform[] mergedBones, DecimationAlgorithm.StatusReportCallback statusCallback = null, bool preserveBorders = false, bool preserveSeams = false, bool preserveFoldovers = false)
		{
			...
			for (int i = 0; i < meshes.Length; i++)
			{
				...
				if (array == null)
				{
					throw new ArgumentException(string.Format("The mesh materials for index {0} is null!", i), "materials");
				}
				if (array.Length != mesh.subMeshCount)
				{
					throw new ArgumentException(string.Format("The mesh materials at index {0} don't match the submesh count! ({1} != {2})", i, array.Length, mesh.subMeshCount), "materials");
				}
				for (int j = 0; j < array.Length; j++)
				{
					if (array[j] == null)
					{
						throw new ArgumentException(string.Format("The mesh material index {0} at material array index {1} is null!", j, i), "materials");
					}
				}
			}
			int num3 = 0;
			...
			for (int k = 0; k < meshes.Length; k++)
			{
				...
				for (int n = 0; n < subMeshCount; n++)
				{
					...
					int value;
					if (dictionary.TryGetValue(material, out value))
					{
						int[] arr = list2[value];
			...
			Matrix4x4[] bindposes2 = ((list11 != null) ? list11.ToArray() : null);
			resultMaterials = list13.ToArray();
			mergedBones = ((list12 != null) ? list12.ToArray() : null);
