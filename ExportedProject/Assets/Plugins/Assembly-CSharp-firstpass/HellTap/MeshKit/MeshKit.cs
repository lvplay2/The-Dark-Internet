...
namespace HellTap.MeshKit
{
	public class MeshKit : MonoBehaviour
	{
		...
		{
			return Strip(m, optimize, false, false, false, false, false, false, false, false, false, false, false, false, false);
		}
			...
			if (mesh != null)
			{
				if (mesh.subMeshCount > 1)
				{
					...
					for (int i = 0; i < mesh.subMeshCount; i++)
					{
						...
						mesh2 = StripUnusedVertices(mesh2, true);
						array[i] = mesh2;
		...
		public IEnumerator CombineChildrenAtRuntime(GameObject go, bool optimizeMeshes, int createNewObjectsWithLayer, string createNewObjectsWithTag, bool enabledRenderersOnly, bool createNewObjectsWithMeshColliders, bool deleteSourceObjects, bool deleteObjectsWithDisabledRenderers, bool deleteEmptyObjects, int userMaxVertices = 65534)
		{
			...
			{
				if (!(meshFilter.sharedMesh == null) && (!(meshFilter.gameObject.GetComponent<MeshRenderer>() != null && enabledRenderersOnly) || meshFilter.gameObject.GetComponent<MeshRenderer>().enabled) && meshFilter.sharedMesh.subMeshCount <= 1)
				{
			...
			if (dictionary.Keys.Count < num)
			{
				...
				{
					float num2 = 0f;
					CombineInstance[] array2 = dictionary[key].ToArray();
					...
					if (num2 < (float)userMaxVertices)
					{
						...
						gameObject.AddComponent<MeshFilter>().mesh.CombineMeshes(dictionary[key].ToArray(), true, true);
					...
					arrayList2.Clear();
					array2 = dictionary[key].ToArray();
					for (int i = 0; i < array2.Length; i++)
					{
						CombineInstance combineInstance2 = array2[i];
						...
						if (num5 + (float)combineInstance2.mesh.vertexCount < (float)userMaxVertices)
						{
							...
							if (num3 == dictionary[key].Count - 1)
							{
								...
								meshFilter3.mesh.CombineMeshes(combine2, true, true);
				...
				while (!foundEmptyObjects)
				{
					...
					Transform[] componentsInChildren4 = go.GetComponentsInChildren<Transform>(true);
