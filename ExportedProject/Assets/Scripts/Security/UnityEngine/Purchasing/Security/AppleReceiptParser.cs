...
namespace UnityEngine.Purchasing.Security
{
	public class AppleReceiptParser
	{
		...
		{
			if (_mostRecentReceiptData.ContainsKey("k_AppleReceiptKey") && _mostRecentReceiptData.ContainsKey("k_PKCS7Key") && _mostRecentReceiptData.ContainsKey("k_ReceiptBytesKey") && ArrayEquals(receiptData, (byte[])_mostRecentReceiptData["k_ReceiptBytesKey"]))
			{
			...
			using (MemoryStream stream = new MemoryStream(receiptData))
			{
				...
				AppleReceipt appleReceipt = ParseReceipt(receipt.data);
				_mostRecentReceiptData["k_AppleReceiptKey"] = appleReceipt;
